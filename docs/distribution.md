...
For a subset, replace `'*'` with names such as `java-implement`. For Claude standalone
...
```sh
gh skill install Dankosik/java-backend-skills java-implement --pin v1.0.0 --agent codex --scope project
...
a namespace, for example `/java-backend-skills:java-implement`. Refresh the catalog and
update this plugin when you choose to adopt a new release:

```sh
claude plugin marketplace update dankosik-skills
claude plugin update java-backend-skills@dankosik-skills
```
...
codex plugin add java-backend-skills@dankosik-skills
```
